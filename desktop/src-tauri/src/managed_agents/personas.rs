...
const REMOVED_BUILT_IN_PERSONA_IDS: &[&str] = &["builtin:fizz", "builtin:honey", "builtin:bumble"];
...
fn merge_personas(mut stored: Vec<AgentDefinition>, now: &str) -> (Vec<AgentDefinition>, bool) {
    ...
    // These legacy starter personas were product defaults, not user-owned
    // records. Remove them even when they were renamed or customized so a
    // stale Fizz/Honey/Bumble catalog cannot survive an upgrade.
    let before = stored.len();
    stored.retain(|record| !REMOVED_BUILT_IN_PERSONA_IDS.contains(&record.id.as_str()));
    ...
    // Soft-deprecate older built-in personas. Runs after demotion so the
